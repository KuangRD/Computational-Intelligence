...
for j in range(candidate_choose_num):
    candidate_choose[].append(trave_list.shuffle())
"""
        ...
        for random_point in random_point_list:
            ...
            for p in range(1, len(fitness_list) - 1):
                if (random_point <= cumulative_prob[p]) and (random_point > cumulative_prob[p - 1]):
                    ...
                    # candidate_choose_list.pop(p)
                    assert len(selected_list[
                                   -1]) == 10, "selected list failed in idx_1+,generation %r,cp %r,sl %r" % generation_idx
                    choose_flag += 1
...
#            print "*********************"
        ...
        distance_list.append(min(fitness_list))

    return candidate_choose_list[fitness_list.index(min(fitness_list))]
