...
def candidater_generator(candidate_choose_num):
    ...
    for i in range(candidate_choose_num):
        ...
        candidate_choose_list1.append(trans_list)
...
def GA1(generation_num):
    ...
    mutation_prob_cumu = 0.0
    now_best_distance = 1000000000000
    now_best_choice = []
    ...
    for generation_idx in range(generation_num):
        ...
        for candidate_choose in candidate_choose_list1:
            fitness_list_origin.append(sum_distance(candidate_choose))


        now_best_choice = candidate_choose_list1[fitness_list_origin.index(min(fitness_list_origin))]
        max_element = max(fitness_list_origin)
        for element in fitness_list_origin:
            fitness_list.append(max_element-element)

        assert len(fitness_list)== candidate_choose_num,"FL initial failed"
        sigma_distance = sum(fitness_list)
        if sigma_distance <1.0:
            sigma_distance = 1.0
            #mutation_rate = mutation_rate * mutation_ratio
           # print "training stop"
        ...
        for random_point in random_point_list:
            ...
            if (random_point <= cumulative_prob[0]):
                selected_list.append(candidate_choose_list1[0])
            ...
            for p in range(1, len(fitness_list) - 1):
                if (random_point <= cumulative_prob[p]) and (random_point > cumulative_prob[p - 1]):
                    ...
                    selected_list.append(candidate_choose_list1[p])
                    # candidate_choose_list.pop(p)
            ...
            if (random_point > cumulative_prob[len(fitness_list)-2]):
                selected_list.append(candidate_choose_list1[9])
                # candidate_choose_list.pop(p)
                #assert len(selected_list[-1]) == 10, "selected list failed in idx_0, generation %r " % generation_idx
                choose_flag += 1
        ...
        while len(candidate_choose_list1) > 0:
            candidate_choose_list1.pop()

        assert len(candidate_choose_list1) == 0,"candidate_choose_list claear failed, generation %r" % generation_idx
        ...
        #    print "Len Selected_list",len(selected_list)
            ...
            c = crossover(a,b)
            candidate_choose_list1.append(c)

            if sum_distance(a) < sum_distance(b):
                candidate_choose_list1.append(a)
            else:
                candidate_choose_list1.append(b)

            q+=1

        assert len(candidate_choose_list1) == candidate_choose_num,"len candidate_choose_list = %r after [crossover],generation %r"  % (len(candidate_choose_list),generation_idx)

        mutation_prob_cumu += mutation_rate * len(candidate_choose_list1) * len(candidate_choose_list1[0])
        #print mutation_prob_cumu

        while mutation_prob_cumu >= 5.0:
            #print "mutation actived"
            idx = random.choice(range(10))
            candidate_choose_list1[idx] = mutation(candidate_choose_list1[idx])
            mutation_prob_cumu =  mutation_prob_cumu-1.0
...
#            print "*********************"
        ...
        #print "Generation",generation_idx,"Distance",min(fitness_list1)
            ...
            #now_best_distance = min(fitness_list1)
        now_best_choice = candidate_choose_list1[fitness_list1.index(min(fitness_list1))]

        distance_list1.append(min(fitness_list1))
...
candidater_generator(candidate_choose_num)
#final_choose1 = GA1(generation_num)
...
#plt.plot(distance_list,'r',distance_list1,'b')
